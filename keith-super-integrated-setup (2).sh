...
setup_project_structure() {
    ...
    # Essential directories for Keith's actual platform structure
    local CONSCIOUSNESS_DIRS=(
        ...
        "components/tribunal"
        "components/bucket-drops"
        "components/musical-dna"
        "components/tapestry"
        "consciousness"
        "tribunal"
        "musical-dna"
...
setup_consciousness_database() {
    ...
    # Create Prisma client (don't overwrite if exists)
...
create_consciousness_apis() {
    ...
    # Create basic tribunal API (don't overwrite if exists)
    if [ ! -f "app/api/tribunal/route.ts" ]; then
        cat > app/api/tribunal/route.ts << 'EOF'
...
EOF
    ...
    # Create bucket drops API (don't overwrite if exists)
    if [ ! -f "app/api/bucket-drops/route.ts" ]; then
        cat > app/api/bucket-drops/route.ts << 'EOF'
